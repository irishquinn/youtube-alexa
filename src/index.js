...
function getSingleVideo(req, res, lang) {
  ...
  if (query == null){
    res.say(response_messages[lang]["NO_QUERY_DEFINED"]).send();
...
function get_video(query, res, lang){
    return new Promise((resolve, reject) => {
      ...
      // Add correct language to search query
      if (lang === "de-DE") {
        search += "?language=de";
      }else if (lang === "fr-FR") {
        search += "?language=fr";
...
function play_list(req, res, lang) {
  ...
  console.log("Searching ... " + query);
  if (query == null){
    res.say(response_messages[lang]["NO_PLIST_QUERY_DEFINED"]).send();
  }else{
    PlayListTitles = set_playList_Array(query);
    if(PlayListTitles.length == 0){
      res.say(response_messages[lang]["NO_PLIST_FOUND"]).send();
    }else{
...
function add_to_play_list(req, res, lang){
  ...
  if (query == null){
    ...
    res.say(response_messages[lang]["ONLY_NUMBER_PLIST_QUERY"]).send();
  }else if(has_video()){
    var fs = require('fs');
    if (fs.existsSync(StorePath + query + '.txt')) {
      ...
      if(isIDexists){
        res.say(response_messages[lang]["ALREADY_IN_PLAYLIST"].formatUnicorn(query));
      }else{
        ...
        res.say(response_messages[lang]["ADD_TO_PLAYLIST"].formatUnicorn(query));
      }
    }else{
      //Create PlayList file if not exists and insert the title ID
      console.log('insert in PlayList content');
...
function remove_from_play_list(req, res, lang){
  ...
  if(isIDexists){
    ...
    res.say(response_messages[lang]["REMOVE_FROM_PLAYLIST"]);
  }else{
    res.say(response_messages[lang]["NOT_IN_PLAYLIST"]);
  }
}
...
function remove_play_list(req, res, lang){
  ...
  if (query == null){
    ...
    if(is_file_removed){
      res.say(response_messages[lang]["PLAYLIST_REMOVED"].formatUnicorn(query)).send();
    }else{
      res.say(response_messages[lang]["NO_PLIST_FOUND"]).send();
    }
  }
...
function remove_all_playlists(req, res, lang){
  ...
  if(isCleared){
    res.say(response_messages[lang]["ALL_PLAYLISTS_REMOVED"]).send();
  }else{
    res.say(response_messages[lang]["NO_PLIST_FOUND"]).send();
  }
}
...
function get_all_playlists(req, res, lang){
  ...
  if(playlists){
    ...
    if(lastComma != 0){
      playlists = playlists.slice(0, lastComma) + response_messages[lang]["AND"] + playlists.slice(lastComma);
    }
    res.say(response_messages[lang]["GET_ALL_PLAYLISTS"].formatUnicorn(playlists)).send();
  }else{
    res.say(response_messages[lang]["NO_PLIST_FOUND"]).send();
  }
}
...
app.intent("AMAZON.StartOverIntent", {}, function(req, res) {
  if (has_video()) {
    ...
    res.say(response_messages[lang]["NOTHING_TO_REPEAT"]);
  }
  res.send();
});
...
app.intent("AMAZON.ResumeIntent", {}, function(req, res) {
  if (is_streaming_video()) {
    ...
    res.say(response_messages[lang]["NOTHING_TO_RESUME"]);
  }
  res.send();
});
...
app.intent("AMAZON.RepeatIntent", {}, function(req, res) {
  ...
  res.say(
    response_messages[lang]["REPEAT_TRIGGERED"]
...
app.intent("AMAZON.LoopOnIntent", {}, function(req, res) {
  ...
  res.say(
    response_messages[lang]["LOOP_ON_TRIGGERED"]
      .formatUnicorn(has_video() ? "current" : "next")
  ).send();
});
...
app.intent("AMAZON.LoopOffIntent", {}, function(req, res) {
  ...
  res.say(
    response_messages[lang]["LOOP_OFF_TRIGGERED"]
      .formatUnicorn(has_video() ? "current" : "next")
  ).send();
});
...
app.intent("AMAZON.HelpIntent", {}, function(req, res) {
  res.say(response_messages[lang]["HELP_TRIGGERED"]).send();
});
